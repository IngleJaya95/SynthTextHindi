...
def get_text_placement_mask(xyz,mask,plane,pad=2,viz=False):
    ...
    """
    waste,contour,hier = cv2.findContours(mask.copy().astype('uint8'),
                                    mode=cv2.RETR_CCOMP,
    ...
    def render_text(self,rgb,depth,seg,area,label,ninstance=1,viz=False):
        ...
        for i in range(ninstance):
            ...
            for idx in reg_range:
                ireg = reg_idx[idx]
                mtt = regions['homography'][ireg]
                det =  np.linalg.det(mtt)
                if(det<0):
                    continue
                try:
            ...
            if  placed:
                ...
                if viz:
                    ...
                    if i < ninstance-1:
                        eval(input(colorize(Color.BLUE,'continue?',True)))                    
        return res
